...
class FetchSQL extends AsyncTask<Void,Void,String> {
	...
	protected void onPreExecute() {
		if (MainActivity.getInstance().getDebug()){
	...
	public static void UpdateTrajectoriesList(ResultSet rsTrajectories){
		try {
			...
			MapMatching.trajectoriesList.clear();
			while(rsTrajectories.next()) {
				...
				String getStricFix = rsTrajectories.getString(8);
				Boolean StrictFix;				
				if (getStricFix.equals("t"))
					StrictFix = true;
				else 
					StrictFix = false;
				...
				MapMatching.trajectoriesList.add(newTrajectory);
				/*
		...
		Log.d(DEBUG,"Close nodes: " + String.valueOf(MapMatching.trajectoriesList.size()));
	}
	...
	protected void onPostExecute(String value) {
		if (MainActivity.getInstance().getDebug()){
			this.endTime = System.nanoTime();
			double msDuration = (endTime-startTime)/1000000;
			Log.d("SQL", "Duration: " + String.valueOf(msDuration) + "ms");
		}
	}
